...
void skipBlank() {      // bỏ qua dấu cách.
  // TODO
  // Done
...
Token* readIdentKeyword(void) {
  ...
  if (count > MAX_IDENT_LEN){
    ...
    // lay kieu cua token 
    // ham checkkeyword tra ve kieu cua keyword hoac TK_NONE
...
Token* getToken(void) {
  ...
  case CHAR_EXCLAIMATION:
        ...
        else token = makeToken(TK_NONE, lineNo, colNo-1);
        readChar();
        return token;
  case CHAR_EQ:
